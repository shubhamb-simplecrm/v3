...
//  */
export const calculateLine = (initialValues, newLineData = {}) => {
  // console.log("lnnnnnnnnn", initialValues, newLineData)
  let productUnitPrice = "";
  initialValues.product_list_price =
    unformat2Number(initialValues.product_list_price) || 0.0;
  initialValues.product_qty =
    initialValues.product_qty !== "" ? initialValues.product_qty || 1 : 1;
  initialValues.product_unit_price = unformat2Number(
    initialValues.product_unit_price,
  );
  initialValues.discount = initialValues.discount || "Percentage";
  // console.log("initialValues.discount", initialValues.discount)
  if (
    initialValues.product_list_price !== null &&
    initialValues.product_discount !== null &&
    initialValues.discount !== null
  ) {
    var listPrice = unformat2Number(initialValues.product_list_price);
    var discount = unformat2Number(initialValues.product_discount);
    var dis = initialValues.discount || "Percentage";
    // console.log('insideCalculate-------', initialValues)
    // console.log("dis", dis, initialValues.product_discount)
    if (dis === "Amount") {
      if (discount > listPrice) {
        initialValues.product_discount = listPrice;
        discount = listPrice;
      }
      productUnitPrice = listPrice - discount;
      initialValues.product_unit_price = format2Number(listPrice - discount);
      // console.log('insideCalculate-------aaaaaaaaaa')
    } else if (dis === "Percentage") {
      if (discount > 100) {
        initialValues.product_discount = 100;
        discount = 100;
      }
      discount = (discount / 100) * listPrice;
      productUnitPrice = listPrice - discount;
      initialValues.product_unit_price = format2Number(listPrice - discount);
      // console.log('insideCalculate-------bbbbbbbbbbb discount', discount, "productUnitPrice", productUnitPrice, "listPrice", listPrice, "initialValues.product_unit_price", initialValues.product_unit_price)
    } else {
      initialValues.product_unit_price = initialValues.product_list_price;
      initialValues.product_discount = "";
      discount = 0;
      // console.log('insideCalculate-------cccccccccccc')
    }
    // initialValues.product_list_price = format2Number(listPrice);
    // initialValues.product_discount_amount = format2Number(-discount, 6);
    // console.log('insideCalculate', initialValues)
  }

  var productQty = 1;
  if (initialValues.product_qty !== null) {
    productQty = unformat2Number(initialValues.product_qty) || 1;
    Quantity_format2Number(initialValues);
    // console.log('insideCalculate-------ddddddddddd')
  }

  var vat = initialValues.vat; //unformatNumber(initialValues.vat, ',', '.');

  var productTotalPrice = productQty * productUnitPrice;

  var totalvat = (productTotalPrice * vat) / 100;

  if (initialValues.vat_amt) {
    productTotalPrice = productTotalPrice + totalvat;
  }

  initialValues.vat_amt = format2Number(totalvat);

  initialValues.product_unit_price = format2Number(productUnitPrice);
  initialValues.product_total_price = format2Number(productTotalPrice);
  var groupid = 0;
  // if (enable_groups) {
  //     groupid = initialValues.group_number;
  // }
  // console.log('insideCalculate-------eeeeeeeeeeee', initialValues)

  groupid = "group" + groupid;
  Object.keys(newLineData).map((col) => {
    newLineData[col].value = initialValues[newLineData[col].name];
  });
  // calculateTotal(groupid);
  // calculateTotal(initialValues);
  return { initialValuesCalculation: initialValues, newLineData };
};
...
export const calculateSingleGroupTotal = (
  ...
  const lineItemData = initialValues;
  let tot_amt = 0;
  let subtotal = 0;
  let dis_tot = 0;
  let tax = 0;
  ...
  lineItemData.map((lineItem) => {
    ...
    let unit = checkNumberIsValid(lineItem["product_unit_price"], 0);
    let deleted = 0;
    ...
    //   tot_amt += parseFloat(list) * parseFloat(qty);
    //   tot_amtGroup += parseFloat(list) * parseFloat(qty);
    // }
  ...
  initialValues["total_amt"] = format2Number(parseFloat(tot_amt));
  initialValues["subtotal_amount"] = format2Number(parseFloat(subtotal));
  initialValues["discount_amount"] = format2Number(parseFloat(dis_tot));

  if (!isGroupedLineItem) {
    ...
    var shippingtax_amt =
      parseFloat(shipping) * (parseFloat(shippingtax) / 100);
    initialValues["shipping_tax_amt"] = format2Number(
      parseFloat(shippingtax_amt),
    );
  ...
  initialValues["tax_amount"] = format2Number(parseFloat(tax));
  ...
  console.log("totalGroupValuesObj", totalGroupValuesObj);
