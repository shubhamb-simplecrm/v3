...
//  */
export const calculateLine = (lineItem) => {
  ...
  totalProductDiscountAmount = productQTY * productDiscount;
  totalVat =
    ((productTotalPrice - totalProductDiscountAmount) * productVAT) / 100;
  productTotalPrice = productTotalPrice - totalProductDiscountAmount + totalVat;

  lineItem["product_qty"] = productQTY;
  lineItem["vat_amt"] = totalVat;
  lineItem["product_total_price"] = parseFloat(productTotalPrice).toFixed(2);
  lineItem["product_list_price"] = parseFloat(productListPrice).toFixed(2);
  lineItem["product_unit_price"] = parseFloat(productUnitPrice).toFixed(2);

  return lineItem;
};

export const calculateTotalGroup = (
  initialValues,
  currenciesRecords = [],
  allFields = [],
  lastCurrency = [],
  ...
  let ship_tax_amt = 0;
  let final_tot_amt = 0;
  let final_tot_amt1 = 0;
  let otherCharges = 0;
  let shippingAmount = 0;
  Object.keys(row).map(function (key, index) {
    console.log(row[key]);
    tot_amt += checkNumberIsValid(
      pathOr(0.0, [key, "meta", "total_amt"], row),
      0.0,
    );
    subtotal += checkNumberIsValid(
      pathOr(0.0, [key, "meta", "subtotal_amount"], row),
      0.0,
    );
    dis_tot += checkNumberIsValid(
      Math.abs(pathOr(0.0, [key, "meta", "discount_amount"], row)),
      0.0,
    );
    console.log("ssssdis_tot", dis_tot);
    ship_tax_amt += checkNumberIsValid(
      pathOr(0.0, [key, "meta", "shipping_tax_amt"], row),
      0.0,
    );
    tax += checkNumberIsValid(
      pathOr(0.0, [key, "meta", "tax_amount"], row),
      0.0,
    );
    sub_tot_tax += checkNumberIsValid(
      pathOr(0.0, [key, "meta", "subtotal_tax_amount"], row),
      0.0,
    );
    final_tot_amt += checkNumberIsValid(
      pathOr(0.0, [key, "meta", "total_amount"], row),
      0.0,
    );
  });

  console.log("initialValues_shippingAmountaxt", tax);
  console.log("initialValues_shippingAmount", initialValues);
  shippingAmount = checkNumberIsValid(initialValues["shipping_amount"], 0);
  let shippingTax = checkNumberIsValid(initialValues["shipping_tax"], 0);
  let shippingTaxAmount =
    (parseFloat(shippingAmount) * parseFloat(shippingTax)) / 100;
  initialValues["shipping_amount"] = shippingAmount;
  initialValues["shipping_tax_amt"] = shippingTaxAmount;
  tax += shippingTaxAmount;

  console.log("dis_tot", dis_tot);
  final_tot_amt1 =
    parseFloat(tax) + parseFloat(shippingAmount) + parseFloat(subtotal);
  initialValues["total_amt"] = parseFloat(tot_amt).toFixed(2);
  initialValues["subtotal_amount"] = parseFloat(subtotal).toFixed(2);
  initialValues["discount_amount"] = -parseFloat(dis_tot).toFixed(2);
  // initialValues["shipping_tax_amt"] = parseFloat(ship_tax_amt).toFixed(2);
  initialValues["tax_amount"] = parseFloat(tax).toFixed(2);
  // initialValues["shipping_c"] = otherCharges.toFixed(2);
  initialValues["subtotal_tax_amount"] = parseFloat(sub_tot_tax).toFixed(2);
  initialValues["total_amount"] = parseFloat(final_tot_amt1).toFixed(2);

  console.log("initaa", initialValues);
  return initialValues;
};
export const calculateSingleGroupTotal = (
  lineItemData,
  isGroupedLineItem = false,
  currenciesRecords = [],
  allFields = [],
  lastCurrency = [],
  changeField = null,
  ...
  //     }
  // }
  console.log("initialValues_shippingAmount___axt", tax);
  subtotal = parseFloat(tot_amt) - parseFloat(dis_tot);
  lineItemData["total_amt"] = format2Number(parseFloat(tot_amt));
  lineItemData["subtotal_amount"] = format2Number(parseFloat(subtotal));
  lineItemData["discount_amount"] = format2Number(parseFloat(dis_tot));
  ...
  totalGroupValuesObj["tax_amount"] = format2Number(parseFloat(tax));
